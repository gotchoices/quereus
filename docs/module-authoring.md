...
| `alterColumn.setCollation` (**PK column**) | re-key / re-validate PK under new collation, **or** (module enforcing the PK physically under a fixed key collation) negotiate accept-when-consistent / reject-when-divergent (`module.ts` setCollation contract) | ✓ re-keys | ✓ negotiated rejection (throws `UNSUPPORTED` on a divergent PK collation; schema-only when the target equals the fixed key collation) |

> The PK-column `setCollation` cell is now a **negotiated rejection**, not a silent divergence. The store's physical key bytes use a fixed table-level key collation K (`StoreTable.encodeOptions`, = `config.collation || 'NOCASE'`), so it resolves the arm via the behavioral *throw `UNSUPPORTED`* contract (rule 4 below) rather than a new capability flag or an engine routing branch — the accept/reject decision depends on the module's per-table K, which the engine does not track. A divergent PK `SET COLLATE` (target ≠ K) throws a sited `UNSUPPORTED`; a consistent one (target == K) is applied schema-only, since forward PK uniqueness is already physically correct under it. The engine-side **logical-enforce** (write-time logical PK scan) and module-side **physical re-key** options the `store-pk-collate-module-capability` work explored remain *documented future enhancements* (`store-pk-collate-logical-enforce`, `store-pk-collate-physical-rekey`), not shipped — `reject` is the store's complete, honest resolution today.

> **The same gap is closed at the CREATE entry point.** A text PK column's declared collation could diverge from K at `CREATE TABLE` just as at `ALTER` — `create table t (x text primary key)` declares the BINARY default while the store enforces K = NOCASE on the key bytes. `StoreModule.create` reconciles each **text** PK column against K with full ALTER symmetry: an *implicit* default (no `COLLATE` clause) is **normalized up to K** so `table_info()` stops lying, while an *explicit* divergent collation (`x text collate binary primary key`) throws the same sited `UNSUPPORTED`. The implicit-vs-explicit distinction is carried by `ColumnSchema.collationExplicit` (set by `columnDefToSchema` only on a `COLLATE` clause — purely additive; the memory module and every other consumer ignore it). The load path (`connect` / rehydrate) applies the same normalization but **never rejects** (a persisted DDL must stay loadable); post-fix this is largely a no-op because a normalized create persists `collate <K>`. Non-text PK columns keep their declared collation — collation governs key bytes only for text. See [`docs/schema.md` § Fixed physical key collation](schema.md) for the store-side detail.
