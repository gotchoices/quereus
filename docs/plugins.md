...
- **Hash pinning gates the load, and is opt-in.** `expectedHash` returning `undefined` means "not pinned" and the load proceeds unchanged; returning a digest that the fetched bytes do not match aborts with a `PluginHashMismatchError` (exported from the package index) *before* the temp file is written, before `onFetched` fires, and before the import — the rejected code never evaluates. A returned value that is not 64 hex characters is a host bug and fails the load, rather than being read as "unpinned", so a typo cannot silently disarm the pin. `dynamicLoadModule` wraps every failure in `Failed to load plugin from <url>: …`, keeping the original on `error.cause`, so a caller distinguishes a mismatch with `error.cause instanceof PluginHashMismatchError`.
  ...
  Setting `PluginRecord.pinned` turns it into a gate: the CLI feeds every pinned record's `sha256` to `expectedHash`, so a mismatch is refused before the module is imported. It is opt-in per plugin (`.plugin install <url> --pin`, or `.plugin pin <name>`), and a pinned record with no recorded hash is a first observation rather than a violation — the next successful load records one and enforcement starts there. `.plugin trust <name> [hash]` records a new expected hash (fetching and digesting the URL without importing it when no hash is given), `.plugin unpin <name>` goes back to warning; both take effect in the same session. A pin that a startup load refuses prints those two remedies instead of the generic load-failure warning, since neither retrying nor waiting resolves a mismatch. No startup failure disables a plugin — see the CLI section below. `pinned` is meaningful only in a host that installed the Node resolver: quoomb-web keeps its own record type and imports `https:` URLs natively, with no verification step.
...
A plugin that fails to load at startup stays enabled and is retried at the next start. A remote plugin fails for reasons that have nothing to do with the plugin — the host was offline, DNS hiccupped, a proxy returned a 5xx — so disabling it on one bad load would drop it silently until the user noticed. The CLI warns and names `.plugin disable <name>` for the case where the plugin really is the problem.

The CLI never overwrites a `plugins.json` it could not use. A file it cannot read, cannot parse, or that parses to something other than a list of plugin records is renamed to `plugins.json.corrupt-<n>` (the lowest `n` free, so repeated corruption does not clobber the previous copy) and the CLI carries on with an empty list — settings are recoverable by hand from the quarantined file rather than lost to the next save. Saves themselves write a temp file and rename it into place, so an interrupted save cannot leave a truncated `plugins.json` in the first place.
