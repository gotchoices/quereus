...
- **`selectAst`** — the parsed body AST; **`columns`** — the explicit MV column list (`mv(a, b)`), when declared; **`insertDefaults`** — the `insert defaults (...)` clause, consumed by the write-through rewrite.
- **`bodyHash`** — `computeBodyHash` over the canonical definition (explicit column list + body + `insert defaults` clause, rendered by `viewDefinitionToCanonicalString`), used by the declarative-schema differ to detect "definition changed → rebuild".
...
| `materialized_view_modified` | `oldObject`, `newObject: TableSchema` (the maintained table) | After `ALTER MATERIALIZED VIEW … SET TAGS` (catalog-only, no re-materialize), or when an `ALTER TABLE/COLUMN RENAME` rewrites a dependent MV body or its `insert defaults` clause |
