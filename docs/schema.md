...
> **Why `ON CONFLICT (<pk>) DO NOTHING`, not `OR IGNORE`, `OR REPLACE`, or wipe-then-reseed?** The earlier implementation ran `DELETE FROM <tbl>` before the inserts, skipping the wipe only when it could detect the table as freshly created by diffing against the in-memory catalog. On a reopen where that catalog was not rehydrated (host-backed row data lives outside the ephemeral catalog), an already-seeded table was misclassified as fresh, the wipe was skipped, and bare `INSERT`s collided with the persisted rows (`UNIQUE constraint failed: <table> PK`). `OR REPLACE` removed the heuristic and fixed the collision, but `OR REPLACE` is delete-then-insert on a conflicting row — so re-seeding a parent table referenced by `ON DELETE CASCADE` children would fire that cascade on every reopen even when the replaced values were byte-for-byte identical. `OR IGNORE` avoids both problems with a point-key probe, but it follows SQLite semantics and silently skips a row on **any** constraint failure — so a malformed seed row (a typo violating a `CHECK`, a missing `NOT NULL` value, a child-side FK to a missing parent) vanished without a diagnostic on the very first apply. Targeting the conflict at the seed table's PRIMARY KEY (`ON CONFLICT (<pk>) DO NOTHING`) keeps the idempotency/no-cascade/no-wipe wins **and** restores error visibility: the `DO NOTHING` suppresses a row only when the conflicting existing row matches the proposed row at the PK columns — i.e. a genuine re-presentation of an already-present seed PK — so any other malformed row aborts the apply with a clear error. This includes a row that duplicates a value on a **secondary** `UNIQUE` index (whose conflicting row has a *different* PK): it now aborts with `UNIQUE constraint failed: …`, on equal footing with `CHECK constraint failed: …`, `NOT NULL constraint failed: …`, and child-FK violations.
