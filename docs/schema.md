...
The imperative `ALTER TABLE … SET TAGS` **and** `ALTER TABLE … ADD TAGS` paths route through the **same** registry at the matching site (`physical-table` / `physical-column` / `physical-constraint`) during plan-build (both carry `stmt.action.tags`, so they share the `setTags` build case), so a misspelled or mis-sited reserved key fails the statement loudly rather than being stored. `DROP TAGS` removes by key and carries no values, so it does **no** reserved-tag validation — dropping a reserved override is legitimate. The sibling `ALTER VIEW` / `ALTER MATERIALIZED VIEW` / `ALTER INDEX … SET TAGS` **and** `… ADD TAGS` statements validate at the `view-ddl` (view / MV) and `physical-index` (index) sites the same way (both carry `action.tags`, sharing the `setTags` build case); their `… DROP TAGS` forms, like the table-level one, carry no values and so do no reserved-tag validation — dropping a reserved override is legitimate.
