...
**Parent-side FK** obligations are enforced **single-source-spine only** — a multi-source or decomposition parent (whose `OLD.*` is not one basis row) routes nothing; on INSERT they are not even collected (an insert cannot orphan a child). The **row-local CHECK / child-side FK / set-level** obligations route through the same `extraConstraints` seam and so reach a decomposition fan-out — both UPDATE *and* INSERT — too, but each is **gated per base op** at the threading site (`buildViewMutation` for the single-source spine and the UPDATE/DELETE fan-out; `buildDecompositionInsert` for the INSERT fan-out — both in `view-mutation-builder.ts`, sharing the one `constraintsForOp` gate): a synthesized constraint rides a base op iff every **write-row** column it references resolves on that op's target-table columns. So a member insert/update that lacks a referenced basis column is not handed a constraint it cannot build (the bug this gate closes), and the obligation lands exactly on the member(s) that *can* introduce a violation. The write-row column set is established per constraint class:

- **Row-local CHECK** gates on **prover-supplied metadata** (`referencedWriteRowColumns`, attached by `collectLensRowLocalConstraints`): the source CHECK's referenced logical columns mapped to their basis columns, mirroring the prover's own row-local classifier. This is deliberately *not* an AST walk, because a row-local CHECK may contain a subquery (Quereus permits it — auto-deferred to commit) carrying a **correlated** bare write-row ref *inside* the subquery; an AST walk would assume a bare subquery-internal ref resolves against the subquery's own FROM and under-collect it, threading the CHECK onto a member that cannot build it. Mapping through the logical→basis projection instead includes exactly the logical columns of the table (the correlated write-row ref) and excludes a foreign subquery ref (`peer.k`); a name-collision (`peer.title` where `title` is also logical) over-collects, which only ever makes the gate *defer* — the safe direction.
- **Child-side FK / set-level** gate on an **AST walk** (`writeRowColumns`): every `NEW.*` / `OLD.*`-qualified column anywhere (including a count / `EXISTS` subquery's correlated side), since those qualifiers are unambiguous wherever they appear. The subquery-internal bare-ref FROM-resolution assumption the walk makes is sound for these classes, whose bare-in-subquery refs are genuine FROM-resolved aliases.

Two consequences follow: a set-level uniqueness CHECK rides only the fan-out op that owns (and can change) the key — a key-unchanged member UPDATE drops it, which is sound since such an update cannot create a duplicate; and a logical row-local CHECK / child-FK spanning columns on **more than one** member resolves on no single member op and is therefore **deferred** on a decomposition write — on INSERT and UPDATE alike, since both run the same per-op gate — while a single-member-resolvable CHECK / FK still rides its member and fires. A dropped constraint is traced via a debug `log`.
...
- **UPDATE of an optional/EAV member with an arbitrary value** — the matched UPDATE evaluates the value in the member's row scope while the materialize INSERT evaluates it over the anchor, so a value that survives both branches must be expressible without a new capture substrate. The two self-contained shapes — **anchor-resolvable** (`set c = a + 1`, collapsed into one `do update` upsert) and **member self-reference** (`set c = c + 1`, `set c = coalesce(c, 0) + 1`, a matched UPDATE for present rows plus a null-substituted, runtime-non-empty-filtered materialize INSERT for absent rows — null-propagating expressions materialize nothing, null→non-null ones do) — are now supported (see *The Default Mapper* § UPDATE). A value that embeds a subquery, reads a *different* member's column, or mixes anchor + self leaves (and any EAV self-reference, which lowers to a subquery) stays deferred onto the shared per-row capture substrate (`unsupported-decomposition-update`).
