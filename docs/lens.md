...
- An uncovered logical column gap-fills from the advertisement's per-column mapping when one is present (richer than name-match). *This gap-fill execution lands in `lens-multi-source-decomposition`*; the protocol specifies the rule and validates conflicts.
