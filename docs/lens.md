...
**Parent-side FK** obligations are enforced **single-source-spine only** — a multi-source or decomposition parent (whose `OLD.*` is not one basis row) routes nothing. The **row-local CHECK / child-side FK / set-level** obligations route through the same `extraConstraints` seam and so reach a decomposition UPDATE's member fan-out too, but each is **gated per base op** at the threading site (`buildViewMutation`): a synthesized constraint rides a base op iff every **write-row** column it references resolves on that op's target-table columns. So a member UPDATE that lacks a referenced basis column is not handed a constraint it cannot build (the bug this gate closes), and the obligation lands exactly on the member(s) that *can* introduce a violation. The write-row column set is established per constraint class:
