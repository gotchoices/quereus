...
- **shipped** — `src/schema/lens-compiler.ts` — the default **name-based aligner** (single-source, v1) **plus the per-attribute sparse-override merger** (override-covered columns ⊕ default-mapper gap-fill ⊖ `hiding`, with the hide-trap / partial-cross-basis-join fidelity boundary erroring rather than guessing) and the **explicit `over Y` basis binding** (which resolves the default-basis ambiguity — `inferDefaultBasis` is consulted only when no lens block is declared). The composition is recomputed (re-read from source) on every `apply schema X` (`runtime/emit/schema-declarative.ts`). `quereus_effective_lens(schema, table)` (`func/builtins/explain.ts`) introspects the result. The **n-way `get` synthesis** (`compileDecompositionBody`, `lens-multi-source-get-synthesis`) consumes a resolved primary advertisement: a left-deep anchor-first equi-join (inner for mandatory / outer for optional members, `on 1 = 1` for the empty-key singleton), EAV pivot columns as correlated scalar subqueries, plus advertisement-driven gap-fill for sparse overrides — all read-direction. The **existence-anchor IND injection** (`lens-multi-source-ind-injection`) shipped — `computeExistenceAnchorInds` records one relation-IND per mandatory member on `LensSlot.injectedInds` for the prover. **pending** — the `put` fan-out + surrogate threading (`lens-multi-source-put-fanout`): a multi-source `get` body is read-correct but still write-rejected (a join body is not a single-source updatable projection); routing attached constraints to enforcement (prover ticket — an MV/lens here is **read-correct, write-unsound** for logical-constraint enforcement).
