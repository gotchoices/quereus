...
Pending (each deferred onto substrate not yet present, raised with a precise diagnostic): **INSERT** (needs the per-row/per-statement shared-surrogate mutation-context envelope — evaluate-once-and-thread — from `view-mutation-shared-surrogate-insert`); a DELETE/UPDATE **`WHERE` that references a non-anchor member** (needs snapshot-consistent multi-member base-op execution — the same substrate the predicate-honest multi-side join delete defers onto); an **optional/EAV/shared-key column UPDATE** (an insert-or-delete of a component row, or an identity change); and **composite shared keys** (v1 threads a single-column key).
