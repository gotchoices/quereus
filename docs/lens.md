...
> - `lens-constraint-enforcement-wiring` (**partially shipped**): the **live per-write enforcement** that consumes the obligations on each mutation. The **row-local class is live** — a logical `check` classified `enforced-row-local` is rewritten from logical→basis column terms and merged into the basis write's per-row `ConstraintCheckNode`, so it fires on every insert/update through the lens even when the basis carries no such check (`planner/mutation/lens-enforcement.ts`, routed by `planner/building/view-mutation-builder.ts`). **Pending** (sibling fix/plan tickets): the set-level existence lookup (`enforced-set-level` row-time via covering structure + commit-time `DeltaExecutor` scan, with `insert or replace` / `or ignore` conflict resolution), the `enforced-fk` commit-time `DeltaExecutor` cross-relation check, and the FD-contribution of proved/enforced keys to the optimizer. Until those land, a set-level / FK write through the lens is enforced only by whatever the basis tables themselves carry; the read-only gate and row-local checks are live.
