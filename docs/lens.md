...
> - `lens-constraint-enforcement-wiring` (pending): the **live per-write enforcement** that consumes the obligations on each mutation (row-local check pipeline, set-level existence lookup, FK `DeltaExecutor`). Until it lands, the obligations below are *classified and recorded* but a write through the lens is enforced only by whatever the basis tables themselves carry; the read-only gate, however, is live.
...
- **shipped** — `src/schema/lens-prover.ts` (`lens-prover-and-attachment`) — `proveLens(slot, db)` proves the compiled body's FD / key / type / nullability surface conforms to the logical spec and **classifies each constraint into an obligation** (`proved` / `enforced-row-local` / `enforced-set-level{row-time|commit-time}` / `enforced-fk` / `vacuous`), recorded on `LensSlot.obligations` + `LensSlot.readOnly`. The five blocking errors + three advisory warnings of the [coverage checklist](#coverage-checklist) are produced here; errors abort the deploy atomically (aggregated, before any catalog mutation), warnings flow to the {@link LensDeployReport} persisted on the `DeclaredSchemaManager` (`getDeployedLensReport`). A non-reconstructible PK deploys the table **read-only** (reads work; mutations error at the lens via `planner/building/view-mutation.ts`). **pending** — the *live per-write* enforcement that consumes the obligations (routing row-local checks into the per-row check pipeline, set-level existence into `findIndexForConstraint`, FK into the commit-time `DeltaExecutor`, the `lens.boundary.attached` marker, and the planner FD-contribution of routed constraints) lands in `lens-constraint-enforcement-wiring`; until it lands, a lens is read-correct and *classified*-for-write-soundness (the deploy blocks unsound schemas and the read-only verdict is enforced), but the row-local/set-level/FK obligations are not yet actively enforced on each write. The advisory acknowledgment / escalation governance that consumes and expands the report ships in the sibling below.
