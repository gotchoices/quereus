...
- **UPDATE of an EAV member with an arbitrary value** — the matched UPDATE evaluates the value in the member's row scope while the materialize INSERT evaluates it over the anchor. For an **optional columnar** member every value shape is now supported: the two self-contained shapes — **anchor-resolvable** (`set c = a + 1`, collapsed into one `do update` upsert) and **member self-reference** (`set c = c + 1`, `set c = coalesce(c, 0) + 1`, a matched UPDATE for present rows plus a null-substituted, runtime-non-empty-filtered materialize INSERT for absent rows) — plus an **arbitrary** value (a subquery, a cross-member column, or a mix of anchor + self leaves) via the **single-identity (anchor-key) per-row capture** that materializes the value once over the planned body and reads it back by the stitch key in both branches (see *The Default Mapper* § UPDATE). Only the **EAV** analogue stays deferred (`unsupported-decomposition-update`): an EAV value column projects as a correlated subquery (so a self-reference is `arbitrary`), and an EAV triple is one-to-many off the anchor key, so the single-identity capture's single-row read-back is not well-defined (the prereq-chained `view-write-decomposition-update-captured-eav` follow-up).
