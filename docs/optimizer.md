...
| `col = <expr>` (single-col RHS)  | one-way FD `<other-col> → col` (no EC, no binding). **Known over-claim:** this FD is currently *ungated* at the table reference, so a narrow `select distinct a, b` over a non-keyed `check (b = a + 1)` table re-derives `{a}` as a phantom key and drops the DISTINCT (wrong results) — open ticket `fd-oneway-determination-key-bag-overclaim`. |
