...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `TableReferenceNode`                      | Seed `key → others` for each declared key (PK + UNIQUE). Additionally seed FDs / EC pairs / constant bindings / `domainConstraints` from declared CHECK constraints **and from assertion-hoisted `not exists (…)` predicates** (cached per `TableSchema`); see *Check-derived contributions* below. The bi-directional value-equality FD `{a ↔ b}` from a `col = col` CHECK / hoisted assertion is folded **only when one endpoint is a genuine declared key** here — over a non-keyed table it would otherwise let a narrow `select distinct a, b` re-derive a phantom all-columns key (ticket `fd-check-assertion-key-bag-overclaim`, mirroring the Filter / Join / Project gates). The EC pair always merges (value-equality is unconditionally sound and ECs are not read by `keysOf`). Bindings are then closed over the resulting EC list. |
...
| `FilterNode`                              | Inherit child. For each equality conjunct: `col = literal` or `col = ?` ⇒ `∅ → col` FD plus a `ConstantBinding`; `col1 = col2` ⇒ EC merge (always) plus the bi-directional determination FDs **only when `col1` or `col2` is a superkey of the filter's input FDs** (else `select a, b from t where a = b` over non-unique a/b would let `deriveKeysFromFds` re-derive a phantom all-columns key — a bag read as a set; ticket `fd-derived-key-bag-overclaim`). **Guard activation** (`activateGuardedFds`): a guarded source FD whose guard the predicate entails is stripped to its unconditional twin — but any activated **single↔single** determination FD `{a}→{b}` is read by `deriveKeysFromFds` as a uniqueness claim, so the fold is gated on endpoint superkey-ness against the filter's input keys (drop the FD when neither endpoint is a key) for **every** single↔single activated FD, exactly like the predicate-derived pair. This covers a guarded **value-equality** body (`status <> 'active' or a = b` → mirror `{a}↔{b}`), a **one-way** guarded determination (`status <> 'active' or b = a + 1` → `{a}→{b}`), and an index-derived guarded pair — all over-claim the same phantom all-columns key once a narrow `select distinct a, b` drops the table's real key. The gate keys off the FD **shape**, NOT the `valueEquality` marker, because `shiftFds`/`projectFds` drop the marker (a value-equality FD activated through a join/projection would otherwise fold ungated — wrong results; tickets `fd-guarded-activation-key-bag-overclaim` site 7 + `fd-oneway-guard-activation-key-bag-overclaim`). Multi-dependent key FDs (`{c}→{id,…}` from a partial UNIQUE) are not single↔single and pass through (a genuinely unique determinant). A genuine value-equality additionally lifts its equality as an EC — and **only** that EC lift keys off the `valueEquality` marker, NOT the FD shape, because a coincidental mutual-determination mirror (two partial UNIQUE indexes on a 2-col table, or `b=a+1`+`a=b-1` checks) is structurally identical but is not an equality, so lifting an EC there would be unsound. Losing the marker (join/projection path) loses only the EC optimization (an under-claim), never soundness. Bindings are then closed over the resulting EC list. |
| `ProjectNode` / `ReturningNode`           | Project FDs/ECs through the source→output mapping built from (a) bare column-reference projections **and** (b) *injectively-derived* projections — scalar expressions that reference exactly one source attribute `a` (with all other leaves being `LiteralNode` / `ParameterReferenceNode`) and satisfy `ScalarPlanNode.isInjectiveIn(a).injective`. The derived column is treated as a synonym of `src(a)`: source keys/FDs/ECs flow through to its output index. Non-injective expressions still drop out. When both `a` and an injective derivation of `a` are projected (`SELECT id, id+1`), the helper copies the unique key onto the derived column, and additionally emits bi-directional FDs between the bare and derived columns **only when one endpoint is a genuine superkey of the projection** (else `SELECT -c, c` over a non-unique `c` would over-claim a phantom all-columns key — ticket `fd-derived-key-bag-overclaim`). Built-in injectivity covers unary `±x`, `x ± const`, `const ± x`, and same-logical-type `CAST`; scalar functions opt in via the `injectiveOnArgs` trait on the `FunctionSchema`. **`isSet` soundness:** a projection that drops a row-distinguishing column turns a set into a bag (`select x from <set on (x,y)>` may repeat `x`), so `getType().isSet` is **not** inherited blindly — it is true only if a declared source key survives the projection, or the source is a set *and* every source column survives (the all-columns key survives). |
...
| `JoinNode` / `BloomJoinNode` / `MergeJoinNode` (inner / cross) | `union(leftFds, shift(rightFds, leftCols))`; for each equi-pair `(L, R')`: EC merge `L ≡ R'` (always) plus bi-directional determination FDs **only when `L` or `R'` is a superkey of the preserved (fan-out-aware) keys** (else a narrow projection of the equi columns would re-derive a phantom key — ticket `fd-derived-key-bag-overclaim`). Constant bindings union both sides (right shifted), then close over the merged EC list — so a one-sided `t.k = 5` plus an equi-pair `t.k = u.k` lands as a single binding covering both columns. Preserved keys (incl. the empty ≤1-row key, see below) are layered on as `key → all_other_join_cols` FDs. |
| Join (left outer)                         | Keep left's FDs/ECs/bindings only; drop right's and equi-pair contributions (NULL-padded rows can violate them). When the left side is **not** preserved (the equi-predicate covers no right key, so the join fans the left side out) its **key** FDs are dropped via `dropSideKeyFds` — extending the inner/cross fan-out rule to LEFT (ticket `fd-derived-key-bag-overclaim`) so a fanned left key cannot resurrect as an all-columns key downstream; non-key / guarded FDs are retained. Preserved left keys (incl. the empty key when both sides are ≤1-row) are layered on. |
...
| `col = <expr>` (single-col RHS)  | one-way FD `<other-col> → col` (no EC, no binding). Like the `col1 = col2` bi-FDs, this single↔single FD is folded at the table reference **only when an endpoint is a real declared key**, else gated — otherwise a narrow `select distinct a, b` over a non-keyed `check (b = a + 1)` table would re-derive `{a}` as a phantom key and drop the DISTINCT (wrong results). Both shapes now share one producer gate (`foldSingleSingleGated`, ticket `fd-oneway-determination-key-bag-overclaim`). |
...
- `hasSingletonFd(fds, columnCount)` — true iff `∅ → all_cols` is present (replaces the `[[]]` marker). The FD-only ≤1-row test that `keysOf` calls internally.
...
**Singleton equivalence.** Three representations encode the same "≤1-row" fact: an empty-key `[]` in `RelationType.keys`, the null-determinant `∅ → all_cols` FD in `physical.fds`, and `isAtMostOneRow` (= `isUnique([])`) returning true. Producers fold the FD through the canonical `addSingletonFd` helper; node-level consumers read `isAtMostOneRow`; and the **Singleton equivalence** property law in the Key Soundness harness (`test/property.spec.ts`) pins the channels to agree. Walking every relational node in the optimized plan it asserts both implications: `isAtMostOneRow(node)` ⇒ `keysOf(node)` contains the empty key `[]`, and `hasSingletonFd(node.physical?.fds, colCount)` ⇒ `isAtMostOneRow(node)`. Because `keysOf` consults `hasSingletonFd` and `isAtMostOneRow` consults `keysOf`, both implications hold *by construction* on today's read surface — so this law is a regression guard against a future refactor of `keysOf` / `isUnique` / `hasSingletonFd` that breaks their reconciliation, **not** a check on producers.
...
- INNER / CROSS: when side B's key is covered, preserves side A's unique keys (both directions can apply) — `propagateJoinFds` materializes each preserved key as a `key → all_other_join_cols` FD on the join output — and caps `estimatedRows` at side A's row count. **Fanning-side key FDs are dropped:** when a side is *not* preserved (no preserved key lies entirely within its columns) the join fans it out, so `propagateJoinFds.dropSideKeyFds` filters out that side's **key** FDs (those whose determinant is an `isSuperkey` of the side) before the merge. The determination still holds under fan-out but no longer encodes uniqueness; carried through it would let a downstream key-dropping projection re-read the side key as a spurious all-columns key (a bag silently read as a set). Non-key and guarded FDs are retained. The genuinely-surviving keys (e.g. the composite product key of a bare cross join) are re-added via the preserved-key list, so nothing sound is lost
- LEFT outer: when the **right** side's key is covered, preserves the **left** side's unique keys and caps `estimatedRows` at left's row count. The right-side keys are NOT propagated — unmatched left rows produce NULL-padded right columns, breaking right uniqueness. If the right key is not covered the left side fans out, so no keys propagate **and** `propagateJoinFds` drops the left side's **key** FDs via `dropSideKeyFds` (mirroring INNER/CROSS — ticket `fd-derived-key-bag-overclaim`); non-key / guarded FDs are retained
