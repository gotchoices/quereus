...
- `ruleSelectAccessPath`: Chooses between sequential scan, index scan, and index seek for both primary and secondary indexes. **Collation cover**: an index seek is only a complete substitute for a predicate when the index column's collation equals the predicate's effective comparison collation (resolved at plan time exactly as `emitComparisonOp` / `emitIn` do at runtime). On a mismatch the rule classifies the cover relation per consumed seek constraint: a *coarser* equality index (a BINARY predicate over a NOCASE/RTRIM index) over-fetches a provable superset, so the seek is kept and the original predicate is re-applied as a residual `Filter` above the leaf; a *finer* index (a NOCASE/RTRIM predicate over a BINARY index, which under-fetches) declines the seek entirely and falls back to a `SeqScan` with the full predicate retained as a residual. **Range / prefix-range / OR_RANGE seeks are held to a stricter rule**: the memory runtime positions a range seek by the index collation but filters the range bounds — and early-terminates the walk — with a *BINARY* comparator (`plan-filter.ts` / `scan-layer.ts`), so a range seek reproduces the predicate only when its effective collation **and** the index collation are both BINARY. Any non-BINARY collation — even one that matches the index — is therefore declined to a `SeqScan` + residual (a matching collation still reorders the walked window relative to the BINARY bound filter, under-fetching variants). Letting non-BINARY range seeks actually be used is tracked by `tickets/backlog/memory-range-seek-collation-bounds`. Effective comparison collation is resolved at plan time exactly as `emitComparisonOp` / `emitIn` / `emitBetween` do at runtime (right-operand precedence; for a `BETWEEN` each bound is resolved independently by the constraint's `op`). The cover logic lives in the rule so it covers every index-style module uniformly (a module that matches constraints to index columns by position alone — e.g. the memory module — never sees collation). See `tickets/complete/index-collation-mismatch-residual-filter`.
