...
| `FilterNode`                              | Inherit child. For each equality conjunct: `col = literal` or `col = ?` ⇒ `∅ → col` FD plus a `ConstantBinding`; `col1 = col2` ⇒ EC merge (always) plus the bi-directional determination FDs **only when `col1` or `col2` is a superkey of the filter's input FDs** (else `select a, b from t where a = b` over non-unique a/b would let `deriveKeysFromFds` re-derive a phantom all-columns key — a bag read as a set; ticket `fd-derived-key-bag-overclaim`). **Guard activation** (`activateGuardedFds`): a guarded source FD whose guard the predicate entails is stripped to its unconditional twin — but a guarded **value-equality** body (`status <> 'active' or a = b`) emits mirror FDs `{a}→{b} [g]` + `{b}→{a} [g]` that `recognizeGuardedBody` tags `valueEquality`; once activated the unconditional `{a}↔{b}` is itself a bag-as-set over-claim, so it is gated on endpoint superkey-ness against the filter's input keys exactly like the predicate-derived pair, and the equality is lifted as an EC instead. The EC lift keys off the `valueEquality` **marker**, NOT the FD shape — a coincidental mutual-determination mirror (two partial UNIQUE indexes on a 2-col table, or `b=a+1`+`a=b-1` checks) is structurally identical but is not an equality, so lifting an EC there would be unsound (ticket `fd-guarded-activation-key-bag-overclaim`, site 7). The *bi-directional* value-equality producer is sealed; the **one-way** guarded determination (`status <> 'active' or b = a + 1`) is still folded ungated after activation and over-claims the same phantom key — open ticket `fd-oneway-guard-activation-key-bag-overclaim` (the guard-activation twin of `fd-oneway-determination-key-bag-overclaim`). Bindings are then closed over the resulting EC list. |
