...
| maintenance writes recorded in the sync change log | the backing host module's decision inside `applyMaintenance`, opted in per table via the reserved tag `quereus.sync.replicate = true` (default **off** — a privileged maintenance write emits no module data events otherwise). The store host queues one local `DataChangeEvent` per realized `BackingRowChange`, so the sync layer records column versions / HLC stamps / tombstones as for an ordinary write; create-fill/refresh stays event-free. |
