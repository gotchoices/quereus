...
- **Shifted shape (rebuild):** a source `alter` can shift the body's output shape — most visibly a `select *` body, whose new source column *interleaves* into the output while the create-time table did not reorder. Stuffing the new rows into the stale table schema would surface body values under the wrong column labels (a latent direct-read corruption) and break the positional table↔body alignment the [join read-rewrite](#join-subsumption) relies on. So refresh **rebuilds the maintained table** (`rebuildBackingTable`: drop + recreate at the new shape via the same `buildBackingTableSchema` → `createBackingTable` → fill path as create). The recreate registers under the **same** table name — still one catalog entry — re-attaching the shape-updated derivation (recomputed `logicalKey`/`ordering`/`sourceTables`) and preserving the live table-level tags across the reshape. The drop/create fire `table_removed`/`table_added` on the table's own name, which invalidate any cached prepared plan scanning it and cascade staleness to any consumer MV over this table. A fill failure (e.g. the reshaped body is duplicate-producing under the new PK) drops the half-built table and leaves the MV `stale` so the next read errors rather than serving an empty relation.
