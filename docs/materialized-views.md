...
- **Value-identical upsert suppression in the bounded-delta arms.** The full-rebuild floor's keyed diff already skips byte-identical rows, but the incremental arms apply an unconditional delete+upsert per changed source row even when the recomputed backing row is byte-identical. Suppressing the no-op (and reporting no effective `BackingRowChange` for it — accurate by definition, since nothing changed) saves cascade work for every consumer and is the echo-prevention prerequisite for change-logged (synced) backings. The skip is collation-aware value comparison against the effective pending row, mirroring `replace-all`'s skip-identical discipline.
