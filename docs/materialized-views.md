...
A body that matches no bounded-delta shape — a fanning or outer join, a set operation, a recursive CTE, a scalar aggregate, a >2-source join, or any other relation-producing body **with a provable key** — is maintained by **full rebuild**: per writing statement, the body is re-evaluated against live mid-transaction source state and the backing's contents are replaced transactionally (a keyed diff against the backing's pending layer, so the delta still commits/rolls-back with the source write and still drives the [MV-over-MV cascade](#mv-over-mv-cascade)). The floor is **designed** to be deferred to a **once-per-statement flush** rather than run per row (see [Synchronous, transactional, per-statement](#synchronous-transactional-per-statement)), so a bulk write rebuilds each affected MV once — *that deferral is not yet wired; the arm currently re-evaluates per source row (correct, not yet amortized)*. This is what makes coverage total: the floor is always sound, so the bounded-delta arms are pure optimizations and never a coverage gate.
...
**Full-rebuild is the one deferred arm.** *(Planned design — the per-statement deferral below is not yet wired: the full-rebuild arm currently re-evaluates the body per source row, correct but not amortized. The deferred-rebuild set lands with the per-statement-deferral ticket.)* The bounded-delta arms apply **per row, immediately** (above) — their O(log n) deltas are cheap and the covering-UNIQUE enforcement scan depends on per-row visibility. A full-rebuild re-evaluates the whole body, so it is **to be** run **once per statement**: the DML generator carries a per-statement *deferred-rebuild set* alongside the `BackingConnectionCache`; a source write whose plan is `'full-rebuild'` marks the MV dirty instead of rebuilding, and the set is flushed at the end-of-statement boundary (after the row loop, **inside** the statement-atomicity savepoint, so a rebuild still rolls back on statement failure). Deferring the floor does **not** violate the enforcement-visibility invariant: a full-rebuild MV is **never a covering structure** (`lookupCoveringConflicts` reads only `'inverse-projection'` backings, which stay per-row-immediate), so nothing reads a full-rebuild backing mid-statement. The flush drains as a **worklist**: a rebuilt producer's effective `BackingRowChange[]` cascades to its consumers (a full-rebuild consumer re-dirties into the same drain; an incremental consumer applies inline), DAG-bounded exactly as the [synchronous cascade](#mv-over-mv-cascade).
...
- **Per-statement deferral of the full-rebuild floor.** The floor currently re-evaluates the body **per source row**; deferring it to a once-per-statement flush (the [deferred-rebuild set](#synchronous-transactional-per-statement)) so a bulk write rebuilds each affected MV once is the next refinement.
