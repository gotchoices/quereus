...
The corresponding output columns then form the **coarsened backing key K'**, which keys the backing under the **output** collations. K' is deliberately *not* a planner key fact — it never enters `keysOf` / `RelationType.keys` (it is not a key of the body relation); it is an MV-create policy with these runtime semantics:
...
1. a table exists at `_mv_<name>` whose module equals the MV's declared backing module (anything else: a different-module table fails the entry with CONSTRAINT — not ours to drop; absence just creates+fills);
...
4. every source the body reads lives in the **same module** as the backing, and every `_mv_` source (MV-over-MV) was itself **adopted** this rehydration (a refilled upstream may hold new content — its dependents refill too; an adopted upstream is unchanged, so trust composes through the fixpoint rounds);
5. the host attested a **clean shutdown** (`importCatalog`'s `trustBackings` option — see below).

Any failed gate falls back to the drop+refill above. Gate 5 exists because "same module ⇒ one atomic commit" does **not** hold even within the store module: coordinators are per table and each KV store commits its own batch, so a crash can leave a source table and a backing divergent inside one module, and the DDL-level gates (2–3) are blind to *content* divergence — an unsound adopt would resurrect crash divergence forever. The store's trust basis is a **clean-shutdown marker** instead: `StoreModule.closeAll` writes a reserved `\x00meta\x00clean_shutdown` catalog entry after every batch has flushed, and `rehydrateCatalog` consumes it (read + immediate delete — single-use) into `trustBackings`. A crash leaves no marker; a second rehydrate without an intervening clean close finds none either — both refill, and divergence self-heals. A provider-level atomic multi-store batch would let adopt drop this gate (parked: `store-atomic-multi-store-commit`).

A body that cannot *plan* during a trusted import does not drop the backing (it errors per-entry with the backing preserved as a plain table) — in the MV fixpoint an MV-over-MV dependent legitimately fails to plan until its upstream's round lands, and the preserved rows are what the retry round adopts. A declared-column arity mismatch (a `select *` body widened under an explicit `mv(a, b)` list) is treated the same way: the entry can never materialize, so it errors per-entry with the backing preserved instead of dropping first. Likewise a post-adopt registration failure (the row-time gate) keeps the backing registered as a plain table rather than destroying durable rows. An adopted MV record is byte-identical to a refilled one (same canonical DDL, same hash formula), so adopt-vs-refill leaves identical catalog bytes, and the adopted backing keeps its persisted `__stats__` row count.

Two known trust caveats:

- **Staleness at close is not attested.** `stale` is runtime state, not persisted: an MV that went stale mid-session (any `table_modified` on a source — an ALTER, even a `create index` — detaches row-time maintenance, so subsequent source writes never reach the backing) and was closed cleanly without a `refresh` passes every DDL-level gate at reopen, and adopt registers the *behind* backing as fresh. Tracked as `mv-adopt-stale-at-close` (gate the marker, or carry the stale set in its payload).
...
- **First-class derivation lifecycle (the "maintained table" direction).** The hidden, engine-owned `_mv_<name>` backing is right for local caches and covering indexes, but a migration target ([Migration](migration.md)) needs the backing to be an ordinary, stably-named, sync-addressable basis table whose *derivation* is an attachment rather than its identity. The pending pieces: stable backing identity (a drop of the MV record must be able to retain the table; refresh's shape rebuild and the differ's body-change path are currently incarnation-destroying), **adopt-on-create** (attach a derivation to an existing table's rows, reconciling by diff instead of refill — the engine-level dual of the rehydrate adopt fast path), **attach/detach verbs** (detach = promote to plain base table, nothing physical changes; attach = demote a base table to maintained), and declarative-differ recognition of attach/detach as non-destructive transitions rather than drop+create.
