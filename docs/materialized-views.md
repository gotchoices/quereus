...
**Known limitation — NULL into a NOT-NULL ordering-seeded PK column.** A body `order by <col>` seeds `<col>` into the backing's *physical* primary key (`computeBackingPrimaryKey`), and a NOT-NULL source column becomes a NOT-NULL physical-PK backing column. A physical-PK column cannot lose NOT NULL (the memory manager refuses to DROP NOT NULL on it, and the reshape masks the doomed loosen), so once the source column drops NOT NULL and later produces a NULL row, a refresh whose recomputed body holds that NULL would store it into a column the backing still declares NOT NULL. Refresh **rejects** this loudly (`CONSTRAINT`, naming the column) rather than storing the contradiction — the trade-off being that the MV **cannot be refreshed** while a source NULL persists in the seeded column; recreate the view without `order by <col>` (or excluding `<col>` from the ordering) to allow nullable values. Two scope caveats: (1) the create/import path is unaffected (it derives the backing NOT-NULL flag from the source at that moment, so a nullable-source ordering column seeds a *nullable* PK column that self-consistently stores NULL — the permitted case, not rejected); (2) this guard covers only the **refresh rebuild** — on a plain projection MV `alter … drop not null` does *not* mark the MV stale, so its **row-time maintenance** stays attached and stores the NULL at the source write *before* any refresh (the primary silent-corruption vector, tracked in `fix/bug-mv-rowtime-null-into-notnull-seeded-pk`). The lasting fix that removes the pinned-NOT-NULL column entirely — expressing body order as a materialized secondary index instead of seeding the physical PK — is `backlog/debt-mv-ordering-seed-to-materialized-index`. Pinned in `materialized-view-refresh-reshape.spec.ts` § *NOT-NULL ordering-seeded PK guard*.
...
Maintenance is driven from the runtime DML write boundary, immediately after each source row is recorded, and applies inside the writing statement's savepoint. Five arms are wired — `'inverse-projection'`, `'residual-recompute'`, `'prefix-delete'`, `'join-residual'`, and `'full-rebuild'` — and a maintenance write into one maintained table cascades into every materialized view reading it.
