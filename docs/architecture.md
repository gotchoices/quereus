...
*   **Materialized Views** — `CREATE MATERIALIZED VIEW` stores a query body as a *keyed derived relation* that is kept consistent with its sources **synchronously, inside the writing transaction** (row-time maintenance), so a materialized view is observably indistinguishable from the plain view it derives from — just served from a stored, primary-keyed backing table. There is one maintenance model and no refresh-policy knob: maintenance is a bounded per-row projection of each changed source row (batched per statement), committed/rolled-back in lockstep with the source write, so there is no asynchronous drift to reconcile. The eligible body shape is correspondingly narrow (single-source, row-preserving, projection-includes-PK) and ineligible bodies are rejected at create rather than served under a weaker contract. Dual-registered (the backing `TableSchema` plus a `MaterializedViewSchema`), read-only to direct DML, round-tripped through the declarative-schema pipeline (`bodyHash`-keyed rebuild on a body change), and the substrate the covering-structure / lens layers build on. See [Materialized Views](materialized-views.md).
...
2.  **Property-Based Tests (`test/property.spec.ts`)**
	...
	*   Currently includes tests for:
		...
		*   **View Round-Trip Laws** *(planned — not yet implemented; tracked by `bx-roundtrip-law-harness`)* — will add the backward-direction soundness net (the dual of the Key Soundness forward net): for each operator in the per-operator backward FD walk, assert the PutGet / GetPut / lineage-agreement law — a round-trip through `get` then `put` leaves source state unchanged, and the backward walk's per-operator result agrees with the forward FD walk it is derived from. Pins the `bx-operator-model-and-roundtrip-laws` design-spike outcome before `view-mutation-plan-node-substrate` threads the surface.
