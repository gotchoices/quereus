...
>
> **RIGHT / FULL — read runs, write-through not yet:** the Quereus runtime now **executes** `right join` and `full join` `select`s. The nested-loop emitter (`runtime/emit/join.ts`) buffers the left side and drives from the right for RIGHT, adding a trailing unmatched-left pass for FULL; coverage is in `test/logic/90.5-unsupported-join-types.sqllogic` (the basic RIGHT/FULL/RIGHT-OUTER/FULL-OUTER rows) and `test/logic/90.5.1-right-full-join-read.sqllogic` (column order, empty-side matrix, no-match, many-to-many fan-out, USING, and `exists … as` flags). **Write-through is still not wired**, so a RIGHT/FULL view remains **read-only**. RIGHT is **excluded from write-through recognition** and the static surfaces report it conservative all-`NO` (the preserved/non-preserved classification is the mirror of LEFT and would otherwise advertise it `is_updatable`). FULL self-conservatizes (no preserved anchor — a non-preserved update there rejects `unsupported-outer-join-update`, since no preserved side can key the materialization). Re-admit RIGHT for write-through — and, once designed, FULL — in the dependent `view-write-right-join-readmit` ticket; the runtime prerequisite this paragraph described is now lifted.
>
> **Still deferred (LEFT):** a non-preserved-**only** insert (no preserved row to attach to) rejects `null-extended-create-conflict`. (The **decomposition optional-member / EAV UPDATE** dual — null→non-null materializes a component, non-null→all-null deletes it — is the decomposition analogue and is now **shipped** as anchor-keyed base ops in `decomposition.ts`; see [Decomposition fan-out](lens.md#the-default-mapper) § UPDATE. A non-constant optional/EAV value is now also supported in two self-contained shapes — **anchor-resolvable** (`set c = a + 1`, collapsed into one `do update` upsert) and **member self-reference** (`set c = c + 1`, `set c = coalesce(c, 0) + 1`, a matched UPDATE for present rows plus a null-substituted, runtime-non-empty-filtered materialize INSERT for absent rows); only an **arbitrary** value (subquery / cross-member / mixed anchor+self, and any EAV self-reference) and a shared-key identity write stay rejected with `unsupported-decomposition-update`.)
