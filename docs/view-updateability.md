...
- `src/planner/mutation/decomposition.ts` — the put fan-out for an n-way decomposition lens (`propagateDecomposition` for DELETE/UPDATE; `analyzeDecompositionInsert` for INSERT; `lens-multi-source-put-fanout` + `lens-multi-source-put-insert-fanout`). Its **backward decisions** — column→member routing, the anchor-resolvable predicate gate — are derived from the threaded plan-node `updateLineage` read through the shared `backward-body.ts` consumer (`analyzeDecomposition` plans the synthesized body once and maps each planned `TableReferenceNode` to its member), *not* a parallel scan of the projection AST: the former `buildViewColMap` projection map and the `collectColumnQualifiers` base-qualifier scan are retired (`classifyColumn` routes off the lineage; the advertisement only disambiguates the deferred EAV / optional / non-identity shapes and supplies the shared-key / pivot metadata). **Landed:** DELETE across every member (anchor-last, anchor-resolvable predicate); UPDATE routed to the mandatory non-EAV member backing each column; **INSERT** anchor-first one-per-member off the shared-surrogate envelope (`integer-auto` surrogate minted once per row, per-row/per-statement, or a logical-tuple PK threaded straight through; optional members gated per-row; EAV triples per supplied attribute; singleton over the empty key) — built by `buildDecompositionInsert` in `view-mutation-builder.ts` (the AST `BaseOp[]` model cannot carry the envelope). An **anchor-resolvable** predicate is now *supported*: a WHERE whose every column is an anchor identity base column *or* a computed mapping whose basis lives on the anchor (`bumped = a + 1` → `a + 1 = 11`) substitutes into a predicate over the anchor's own base columns, which the anchor key subquery already evaluates. Still deferred onto absent substrate: a **genuine non-anchor-member / EAV-pivot / embedded-subquery** predicate (`unsupported-decomposition-predicate`, snapshot-consistent multi-member execution — each of the three is a distinct case raising its own accurate message under the shared reason code, so an EAV / subquery predicate is no longer misattributed as a "non-anchor member"), an optional/EAV/key UPDATE transition (`unsupported-decomposition-update`, per-row insert-or-delete branching), non-integer surrogate generators (`no-default`), and composite shared keys (`unsupported-decomposition-key`). Two **robustness guards** reject defensively rather than misroute silently: a self-decomposition (two members over one physical base relation, so column→member routing is ambiguous — `unsupported-decomposition-member`), and an identity column whose lineage resolves a base column but whose base relation is absent from the member map (a lineage-resolution miss — `no-base-lineage`, which must not silently degrade a writable column to read-only).
