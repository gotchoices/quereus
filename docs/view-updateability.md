...
> Outer-join write-through is part of the model above but is not yet wired in the multi-source substrate (which accepts n-way **inner** equi-joins today — composite-PK sides and self-joins included, but no outer joins); an outer-join body is currently rejected wholesale. See [Current limitations](#current-limitations).
...
- **Outer-join / optional-member write-through** — the model is specified under [Outer Joins](#outer-joins), but the multi-source substrate accepts only **inner** equi-joins today (n-way, composite-PK, self-join included — but no outer joins), so an outer-join body is rejected wholesale (both sides).
- **Cross-source `set` values** — an `update` value that references a base column on a *different* side than the column it assigns cannot be expressed as a single-table SET; rejected with `cross-source-assignment`. (n-way / composite-PK / self-join inner-join `update` / `delete` are **now supported** — the key capture carries one column per side per PK column, and routing is alias-keyed.)
