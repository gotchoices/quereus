...
- **`ctxBody`** — the existing target-**excluded** context (`contextForCteTarget`). `analyzeView` plans the body and `buildCteSelfCapture` builds the capture source under it, so the body's own `from base` reaches the **real** base table (the shadow case above is preserved, *including* combined with a self-read).
...
- **Forward-reference shadowing a real table** — a target body that reads a sibling CTE defined *later* whose name also shadows a real base table (`with x as (select … from foo), foo as (…) update x …`) rejects (`CTEReference … not updateable in phase 1`) rather than writing through to the real `foo`. Per SQL scoping `x.foo` should bind the outer base table (a CTE is visible only to *later* siblings), but the CTE-target re-plan threads *all* siblings into `cteNodes` (`contextForCteTarget`), so `foo` resolves to the later CTE. The flattener correctly treats it as terminal (only prior siblings inline); the reject originates in the re-plan context. Not silently wrong (clean reject, never the wrong table). Tracked by backlog ticket `cte-dml-forward-reference-visibility`.
