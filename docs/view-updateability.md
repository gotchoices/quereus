...
- **n-way decomposition fan-out** — driven by `quereus.lens.decomp.*` tags: a columnar split with an optional (outer-joined) member, an EAV pivot member, and a surrogate split; INSERT (anchor-first, per-row optional gate, EAV triples, surrogate minted once and threaded into every member), UPDATE routed to the backing member — including the **optional-member / EAV value-write materialization** (a constant value: matched UPDATE / `on conflict do nothing` materialize INSERT / all-null DELETE; an **anchor-resolvable** value: one `on conflict do update set col = excluded.col` upsert; a **member self-reference**: matched UPDATE for present rows + a null-substituted, runtime-non-empty-filtered materialize INSERT for absent rows; an **arbitrary optional-columnar** value — subquery / cross-member / mixed anchor+self — via the **single-identity (anchor-key) per-row capture**: materialized once over the planned body into `__vmupd_keys`, read back by the member key in the matched UPDATE and the anchor key in a runtime-non-null-filtered materialize INSERT, pre-mutation so a both-sides `set c = b + 1, b = b + 100` reads the pre-mutation `b`), anchor-last DELETE. Deferred shapes (shared-key identity write, an **arbitrary EAV** value — subquery / cross-member / mixed / EAV self-reference, non-anchor predicate, composite key) are asserted to *reject*.
