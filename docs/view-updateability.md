...
> This plan-time ON-DELETE / mutual-FK analysis is **two-side only**. An n-way (>2) delete fan-out orders its chosen sides by the FK topological sort (parent before child) and defers any mutual-FK cycle wholesale to the **runtime** RESTRICT pre-check on the actual data — the plan-time `mutual-fk-restrict-delete` reject is not generalized past two sides (a deliberate scope boundary; see the inner-join substrate).
...
> **Shipped (statically-expressible cases):** the multi-source substrate admits LEFT/RIGHT outer-join bodies and wires the cases the static base-op fan-out can express — preserved-side update passthrough, delete-to-the-preserved-side (an unmatched/null-extended row is deletable too), and insert routing (both-side under the minted shared key, preserved-only producing a null-extended row, and a non-preserved side as a presence-gated optional member of the envelope fan-out). The static `view_info` / `column_info` surfaces report these per-side: a preserved column is `is_updatable = 'YES'`, a non-preserved column `'NO'`, and the view is `is_insertable_into` / `is_deletable`.
>
> **Deferred:** an UPDATE of a **non-preserved** column (the per-row matched-update / null-extended-insert branch) rejects with `unsupported-outer-join-update` (see `view-write-optional-member-transitions`); a `full outer join` (no preserved anchor side — every side null-extended per row) is rejected wholesale (`unsupported-join` on insert/delete, `unsupported-outer-join-update` on update), and the static surfaces report it conservative; a non-preserved-**only** insert (no preserved row to attach to) rejects `null-extended-create-conflict`.
...
interface MutationDiagnostic {
  reason:
    ...
    | 'null-extended-create-conflict'   // outer-join insert supplies only non-preserved columns (no preserved anchor)
    | 'unsupported-outer-join-update'    // update of a non-preserved (null-extended) outer-join column (per-row materialization deferred)
...
- **Non-preserved-side outer-join update** — the LEFT/RIGHT outer-join *statically-expressible* cases are shipped (preserved-side update, delete-to-preserved, insert routing — see [Outer Joins](#outer-joins)). What remains deferred is the per-row matched-update / null-extended-insert branch an UPDATE of a **non-preserved** column needs (`unsupported-outer-join-update`, owned by `view-write-optional-member-transitions`); a non-preserved-**only** insert (`null-extended-create-conflict`); and a **full outer join**, whose every-side-null-extended shape has no preserved anchor (rejected wholesale, surfaces report it conservative).
