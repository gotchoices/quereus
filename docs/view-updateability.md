...
- **Non-preserved-side outer-join update** — the **LEFT** outer-join *statically-expressible* cases are shipped (preserved-side update, delete-to-preserved, insert routing — see [Outer Joins](#outer-joins)). What remains deferred is the per-row matched-update / null-extended-insert branch an UPDATE of a **non-preserved** column needs (`unsupported-outer-join-update`, owned by `view-write-optional-member-transitions`); a non-preserved-**only** insert (`null-extended-create-conflict`); a **full outer join**, whose every-side-null-extended shape has no preserved anchor (rejected wholesale, surfaces report it conservative); and **RIGHT joins entirely**, which the runtime cannot execute (`runtime/emit/join.ts`, pinned by `90.5-unsupported-join-types`) — excluded from recognition and reported conservative until runtime RIGHT-join support lands.
