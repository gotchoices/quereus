...
The lens chain functions without it: storage-shape (writes, the correctness-critical direction) lands via `lens-multi-source-decomposition`; reads over a decomposition still work via the synthesized join + the standard optimizer (just without exotic-access-path selection — a spatial query scans rather than using the nd-tree). This is a read-path *optimization* over an already-correct foundation, and it is a substantial planner/path-selection effort orthogonal to the storage protocol. Promote to plan once `lens-multi-source-decomposition` has landed and a concrete exotic-access module (nd-tree) exists to test against.
