...
### Richer incremental MV bodies — the unified maintenance substrate
Row-time MV maintenance currently rejects: fanning (non-1:1) keyed joins, **outer** 1:1 joins, partial-`WHERE` join bodies, and scalar (no-`GROUP BY`) aggregates. The doc-flagged direction (`docs/materialized-views.md` § Current limitations) is a **unified maintenance substrate** folding the row-time inverse-projection path and the post-commit `DeltaExecutor` binding kernel under one `MaintenancePlan` abstraction with a backward (maintenance-direction) cost gate. DBSP/Materialize-style generality (incl. eventual semi-naïve recursion) is the borrow target. Existing per-shape tickets: `materialized-view-incremental-join-bodies`, `-set-ops`, `-recursive-cte`, `-tvf-sources` (complete/known).
