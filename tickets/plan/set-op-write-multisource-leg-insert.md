...
## Promotion note (2026-06-14): decision-free — prereq satisfied

Promoted backlog→plan during interactive triage. `set-op-write-multisource-leg-compose`
(the prereq) is **complete** (`tickets/complete/`), so the inner per-branch base-PK
capture this lift builds on exists. The one "core design question" in the body — how a
per-leg `buildMultiSourceInsert` envelope composes with the outer set-op
`ViewMutationNode` (carried sub-plans the outer node sequences vs. nested splice) — is an
**internal planner-composition choice for the plan pass to resolve**, not a user-facing
semantic decision: both routes have the same observable contract (the requirements +
validation are fully pinned, including the `is_insertable_into` flip and the 93.4/93.6
coverage flips). No question for the dev.
