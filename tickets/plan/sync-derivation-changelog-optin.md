...
## Promotion note (2026-06-14): decision-free — spec pinned, prereqs satisfied

Promoted backlog→plan during interactive triage. Both prereqs are **complete**:
`mv-noop-upsert-suppression` (`tickets/complete/4-mv-noop-upsert-suppression.md`, the
echo-safety contract) and `store-backing-host` (`tickets/complete/store-backing-host.md`,
the host that reads the tag). The design is **pinned in `docs/migration.md`** — § Synced
vs. local derived tables, the table row "maintenance writes recorded in the sync change
log → the backing host module's decision inside `applyMaintenance`, opted in per table via
a reserved tag (default **off**)". No open user decision: the reserved-tag key is
mechanical (typed registry), default-off keeps every existing MV inert, and the echo loop
is closed by the already-shipped no-op suppression. The plan pass implements the pinned
spec.
