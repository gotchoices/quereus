description: APPETITE SIGN-OFF NEEDED. Make a flag-less set-op body (a flat `union all` of literal-discriminator legs) writable via predicate-honest branch dispatch — the "projected-attribute idiom". Design is fully resolved below and an implement plan is ready to promote; the one thing the plan pass cannot decide is whether the dev wants to re-open the deliberate "membership columns replace routing-tag dispatch" decision and carry a second set-op write path. Blocked on that yes/no, NOT on missing prereq code.
prereq:
