...
**Atomic ALTER** — DDL through Quereus is not transaction-scoped and the shared underlying base auto-commits its mutation immediately, so a half-applied ALTER cannot be rolled back. `IsolationModule.alterTable` therefore dry-run **validates every affected overlay's backfill** (per-row `NOT NULL` checks and the tombstone-present guard) *before* mutating the underlying. A backfill rejection fires while the underlying, the schema catalog, and every overlay are still untouched, so an ADD COLUMN either fails clean or fully applies — base and catalog never diverge.
